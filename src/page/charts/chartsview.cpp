    ...
    , m_binaryTail(QByteArray::fromHex("0000807f"))
{
    ...
    lineChartController->setMenu(lineChartView->settingsMenu());
...
void ChartsView::run()
{
    ...
    connect(this, &ChartsView::input2run, timer, [&tmp, this, isTestData](const QByteArray &bytes) {
        if (!isTestData) {
            tmp.append(bytes);
        }
    });
...
QByteArray ChartsView::handleBinaryY()
{
    QByteArray bytes;
    for (int i = 0; i < 16; ++i) {
...
QByteArray ChartsView::handleTextY()
{
    QString str;
    for (int i = 0; i < 16; ++i) {
...
QByteArray ChartsView::handleBinaryXY()
{
    QByteArray bytes;
    for (int i = 0; i < 16; ++i) {
        float y = 10 * (i + 1) * qSin(m_testAngle * 3.1415926 / 180.0);
...
QByteArray ChartsView::handleTextXY()
{
    QByteArray bytes;
    for (int i = 0; i < 16; ++i) {
        qreal y = 10 * (i + 1) * qSin(m_testAngle * 3.1415926 / 180.0);
...
void ChartsView::handleBinaryY(QByteArray &bytes)
{
...
void ChartsView::handleTextY(QByteArray &bytes)
{
...
void ChartsView::handleBinaryXY(QByteArray &bytes)
{
    while (bytes.indexOf(m_binaryTail) != -1 && !bytes.isEmpty()) {
        int index = bytes.indexOf(m_binaryTail);
...
void ChartsView::handleTextXY(QByteArray &bytes)
{
