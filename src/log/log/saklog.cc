...
SAKLog::SAKLog(QObject* parent) : QThread(parent), mMaxTemp(10240) {
  QDir dir(logPath());
  if (!dir.exists()) {
  ...
  connect(mTableModel, &SAKTableModel::invokeGetRowCount, this,
          &SAKLog::onInvokeGetRowCount, Qt::DirectConnection);
  connect(mTableModel, &SAKTableModel::invokeGetColumnCount, this,
          &SAKLog::onInvokeGetColumnCount, Qt::DirectConnection);
  connect(mTableModel, &SAKTableModel::invokeGetData, this,
          &SAKLog::onInvokeGetData, Qt::DirectConnection);
  connect(mTableModel, &SAKTableModel::invokeSetData, this,
          &SAKLog::onInvokeSetData, Qt::DirectConnection);
  connect(mTableModel, &SAKTableModel::invokeInsertRows, this,
          &SAKLog::onInvokeInsertRows, Qt::DirectConnection);
  connect(mTableModel, &SAKTableModel::invokeRemoveRows, this,
          &SAKLog::onInvokeRemoveRows, Qt::DirectConnection);
  connect(mTableModel, &SAKTableModel::invokeGetHeaderData, this,
          &SAKLog::onInvokeGetHeaderData, Qt::DirectConnection);
}
...
QAbstractTableModel* SAKLog::tableModel() { return mTableModel; }
...
void SAKLog::onInvokeGetColumnCount(int& count) { count = 3; }

void SAKLog::onInvokeGetData(QVariant& data, const QModelIndex& index,
                             int role) {
...
void SAKLog::onInvokeSetData(bool& result, const QModelIndex& index,
...
void SAKLog::onInvokeInsertRows(bool& result, int row, int count,
...
void SAKLog::onInvokeRemoveRows(bool& result, int row, int count,
                                const QModelIndex& parent) {
  Q_UNUSED(parent)
  mTempMutex.lock();
...
void SAKLog::onInvokeGetHeaderData(QVariant& data, int section,
  ...
  if (QFile::exists(file.fileName()) && (file.size() >= 1024 * 1024)) {
    ...
    if (QFile::rename(fileName + suffix, newName)) {
      qCInfo(mLoggingCategory)
          << file.fileName() << "has been rename to:" << newName;
    } else {
