    ...
    , mMaxTemp(10240)
{
    ...
    if (!dir.exists()) {
        if (dir.mkdir(logPath())) {
            qInfo() << "create log dir successfully.";
        } else {
            qInfo() << "create log dir failed!";
        }
    } else {
        qInfo() << "the log dir is exists,"
                   " need not to create.";
    }
...
void SAKLog::appendLog(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
...
void SAKLog::setLogLifeCycle(int t)
{
    ...
    if (logLifeCycle < 1) {
        logLifeCycle = 1;
        qWarning() << "The life cycle is too short,"
                      " it has been set to 1 day.";
    }

    if (logLifeCycle > 30) {
        logLifeCycle = 30;
        qWarning() << "The life cycle is too long,"
...
void SAKLog::onInvokeGetData(QVariant& data, const QModelIndex& index, int role)
{
    ...
    if (row >= 0 && row < mTemp.count()) {
        ...
        if (column >= 0 && column < 3) {
            ...
            if (column == 0) {
                ...
                qWarning() << "Index is invalid(column).!";
            }
        } else {
            qWarning() << "Index is invalid(column)!";
        }
    } else {
        qWarning() << "Index is invalid(row)!";
    }
    mTempMutex.unlock();
}

void SAKLog::onInvokeSetData(bool& result, const QModelIndex& index, const QVariant& value, int role)
{
    Q_UNUSED(role)
    int column = index.column();
    int row = index.row();
...
void SAKLog::onInvokeInsertRows(bool& result, int row, int count, const QModelIndex& parent)
{
    Q_UNUSED(parent)
    mTempMutex.lock();
    if (row >= 0 && row <= mTemp.count()) {
...
void SAKLog::onInvokeRemoveRows(bool& result, int row, int count, const QModelIndex& parent)
{
    Q_UNUSED(parent)
    mTempMutex.lock();
...
void SAKLog::writeLog()
{
    ...
    if (QFile::exists(file.fileName()) && (file.size() >= 1024 * 1024)) {
        ...
        if (QFile::rename(fileName + suffix, newName)) {
            qInfo() << file.fileName() << "has been rename to:" << newName;
        } else {
            qInfo() << "old name:" << fileName;
            qInfo() << "new name:" << newName;
            qInfo() << "rename file failed!";
        }
    }
