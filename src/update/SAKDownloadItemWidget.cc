...
void SAKUpdateManager::setupDownloadList(UpdateInfo info)
{
     ...
     * 清空弄列表
     */
    for(int i = 0; i < downloadListListWidget->count();i ++){
        QListWidgetItem *item = downloadListListWidget->item(i);
    ...
    for(auto var:info.browserDownloadUrl){
        if (var.contains(QString(".exe"))){
            new QListWidgetItem(QIcon(":/resources/images/Windows.png"), var.split("/").last(), downloadListListWidget);
        }
    }

    downloadListListWidget->addItem(tr("源码"));
    new QListWidgetItem(QIcon(":/resources/images/Gz.png"), "tar.gz", downloadListListWidget);
    new QListWidgetItem(QIcon(":/resources/images/Zip.png"), "zip", downloadListListWidget);
}
