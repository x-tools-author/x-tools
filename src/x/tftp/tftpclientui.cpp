    ...
    , m_tftpClient(new TftpClient(this))
{
    ...
    connect(m_tftpClient, &TftpClient::transferCompleted, this, &TftpClientUi::handleTransferCompleted);
...
{
    QString path = QFileDialog::getExistingDirectory(this, "Select Local Directory", ui->lineEditLocalPath->text());
    if (!path.isEmpty()) {
...
void TftpClientUi::on_pushButtonUpload_clicked()
{
    ...
    if (m_tftpClient->uploadFile(localFile, remoteFile)) {
        log(QString("Upload started: %1 to %2:%3 as %4").arg(localFile).arg(serverAddr).arg(port).arg(remoteFile));
        ui->pushButtonDownload->setEnabled(false);
        ui->pushButtonUpload->setEnabled(false);
        ui->pushButtonStop->setEnabled(true);
    } else {
...
void TftpClientUi::handleTransferCompleted(const QString &fileName)
{
    ...
    QMessageBox::information(this, "TFTP Complete", QString("File transfer completed: %1").arg(fileName));
...
bool TftpClientUi::validateInput() const
{
    if (ui->lineEditServerAddr->text().isEmpty()) {
        QMessageBox::warning(const_cast<TftpClientUi*>(this), "Input Error", "Please enter server address");
        return false;
    }
    ...
    if (!portOk || port == 0) {
        QMessageBox::warning(const_cast<TftpClientUi*>(this), "Input Error", "Please enter valid port");
        return false;
    }
    
    if (ui->lineEditLocalPath->text().isEmpty()) {
        QMessageBox::warning(const_cast<TftpClientUi*>(this), "Input Error", "Please select local path");
        return false;
    }
    
    if (ui->lineEditRemoteFile->text().isEmpty()) {
        QMessageBox::warning(const_cast<TftpClientUi*>(this), "Input Error", "Please enter remote file name");
        return false;
    }
