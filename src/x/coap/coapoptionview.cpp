...
namespace Ui {
class CoAPOptionView;
};

namespace xCoAP {

class CoAPOptionViewPrivate : public QObject
{
public:
    CoAPOptionViewPrivate(CoAPOptionView* q_ptr)
        : QObject(q_ptr)
        , q(q_ptr)
    {
        ui = new Ui::CoAPOptionView();
        ui->setupUi(q);
    ...
    ~CoAPOptionViewPrivate() override {}

public:
    Ui::CoAPOptionView* ui{nullptr};
    CoAPMsgModel* m_model{nullptr};
    QMenu* m_columnsMenu{nullptr};

private:
    CoAPOptionView* q{nullptr};
};

struct CoAPOptionViewParameterKeys
{
    const QString columns{"columns"};
};

CoAPOptionView::CoAPOptionView(QWidget* parent)
    : QWidget(parent)
{
    d = new CoAPOptionViewPrivate(this);
}

CoAPOptionView::~CoAPOptionView() {}

QJsonObject CoAPOptionView::save()
{
    CoAPOptionViewParameterKeys keys;
    QJsonObject obj;
...
void CoAPOptionView::load(const QJsonObject& obj)
{
    CoAPOptionViewParameterKeys keys;
