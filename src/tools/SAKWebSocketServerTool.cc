...
bool SAKWebSocketServerTool::initialize(QString &errStr) {
  ...
  mWebSocketServer =
      new QWebSocketServer(serverName, QWebSocketServer::NonSecureMode);
  ...
  connect(mWebSocketServer, &QWebSocketServer::serverError, mWebSocketServer,
  ...
  connect(
      mWebSocketServer, &QWebSocketServer::newConnection, mWebSocketServer,
      [=]() {
        ...
        connect(client, &QWebSocket::textMessageReceived, client,
                [=](const QString &message) {
                  ...
                  quint16 port = client->peerPort();
                  QString ipport =
                      QString("%1:%2").arg(ip, QString::number(port));
        ...
        connect(client, &QWebSocket::binaryMessageReceived, client,
        ...
        connect(client, static_cast<WS_ERR_SIGNAL>(&QWebSocket::errorOccurred),
#else
        connect(client, static_cast<WS_ERR_SIGNAL>(&QWebSocket::error),
                ...
                client, [=](QAbstractSocket::SocketError err) {
                  ...
                  outputMessage(
                      QtInfoMsg,
                      QString("Error occurred: %1").arg(client->errorString()));
                  emit clientsChanged();
                });
      });
...
void SAKWebSocketServerTool::writeBytesInner(QWebSocket *client,
  ...
  if (ret == -1) {
    ...
    outputMessage(QtInfoMsg,
                  QString("%1->%2:%3").arg(this->mBindingIpPort, ipport, hex));
