...
SAKUdpTransmitterTool::SAKUdpTransmitterTool(QObject *parent)
    : SAKTransmitterTool{"SAK.UdpTransmitterTool", parent}
{

}

QVariant SAKUdpTransmitterTool::itemContext(int index)
{
    QJsonObject obj;
...
int SAKUdpTransmitterTool::columnCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent)
    return 6;
}

QVariant SAKUdpTransmitterTool::data(const QModelIndex &index, int role) const
{
...
bool SAKUdpTransmitterTool::setData(const QModelIndex &index,
...
bool SAKUdpTransmitterTool::insertRows(int row, int count,
                                       const QModelIndex &parent)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        connect(this, &SAKUdpTransmitterTool::bytesInputted,
                tool, &SAKUdpClientTool::inputBytes);
        connect(tool, &SAKUdpClientTool::bytesOutputted,
                this, &SAKUdpTransmitterTool::bytesOutputted);
        connect(this, &SAKUdpTransmitterTool::started,
                tool, [=](){tool->start();});
        connect(this, &SAKUdpTransmitterTool::finished,
                tool, [=](){tool->exit();});
...
QVariant SAKUdpTransmitterTool::headerData(int section,
