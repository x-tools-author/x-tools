...
SAKToolBox::SAKToolBox(QObject* parent) : QObject{parent} {
  ...
  mToolList << mTxMaskerTool << mRxMaskerTool << mTxAnalyzerTool
            << mRxAnalyzerTool << mEmitterTool << mResponserTool << mStorerTool
            << mPrestorerTool << mRxVelometerTool << mTxVelometerTool
            << mRxStatisticianTool << mTxStatisticianTool << mUdpTransmitterTool
            << mTcpTransmitterTool << mWebSocketTransmitterTool
            << mSerialPortTransmitterTool;

  int flag = Qt::AutoConnection | Qt::UniqueConnection;
  for (auto tool : mToolList) {
    connect(tool, &SAKBaseTool::errorOccured, this, &SAKToolBox::errorOccurred,
...
void SAKToolBox::initialize(int type) {
  ...
  if (!mComunicationTool) {
    qCWarning(mLoggingCategory)
        << "mComunicationTool is nullptr, type:" << type;
    return;
  }

  // rx->output_masker->output_analyzer->emmitter,responser
  connect(mComunicationTool, &SAKBaseTool::bytesOutputted, mRxMaskerTool,
          &SAKBaseTool::inputBytes);
  connect(mRxMaskerTool, &SAKBaseTool::bytesOutputted, mRxAnalyzerTool,
          &SAKBaseTool::inputBytes);
  connect(mRxAnalyzerTool, &SAKBaseTool::bytesOutputted, mEmitterTool,
          &SAKBaseTool::inputBytes);
  connect(mRxAnalyzerTool, &SAKBaseTool::bytesOutputted, mResponserTool,
          &SAKBaseTool::inputBytes);

  // emiiter,responser,prestorer->input_analyzer->input_masker->tx
  connect(mEmitterTool, &SAKBaseTool::bytesOutputted, mTxAnalyzerTool,
          &SAKBaseTool::inputBytes);
  connect(mResponserTool, &SAKBaseTool::bytesOutputted, mTxAnalyzerTool,
          &SAKBaseTool::inputBytes);
  connect(mPrestorerTool, &SAKBaseTool::bytesOutputted, mTxAnalyzerTool,
          &SAKBaseTool::inputBytes);
  connect(mTxAnalyzerTool, &SAKBaseTool::bytesOutputted, mTxMaskerTool,
          &SAKBaseTool::inputBytes);
  connect(mTxMaskerTool, &SAKBaseTool::bytesOutputted, mComunicationTool,
          &SAKBaseTool::inputBytes);

  // rx->storer; tx->storer
  connect(mRxAnalyzerTool, &SAKBaseTool::bytesOutputted, mStorerTool,
          &SAKBaseTool::inputBytes);
  connect(mTxMaskerTool, &SAKBaseTool::bytesInputted, mStorerTool,
          &SAKBaseTool::inputBytes);

  // rx->velometer; tx->velometer
  connect(mComunicationTool, &SAKBaseTool::bytesOutputted, mRxVelometerTool,
          &SAKBaseTool::inputBytes);
  connect(mComunicationTool, &SAKBaseTool::bytesInputted, mTxVelometerTool,
          &SAKBaseTool::inputBytes);

  // rx->statistician; tx->statistician
  connect(mRxAnalyzerTool, &SAKBaseTool::bytesOutputted, mRxStatisticianTool,
          &SAKBaseTool::inputBytes);
  connect(mComunicationTool, &SAKBaseTool::bytesInputted, mTxStatisticianTool,
          &SAKBaseTool::inputBytes);
  ...
  connect(mRxAnalyzerTool, &SAKBaseTool::bytesOutputted, mUdpTransmitterTool,
          &SAKBaseTool::inputBytes);
  connect(mUdpTransmitterTool, &SAKBaseTool::bytesOutputted, mTxAnalyzerTool,
          &SAKBaseTool::inputBytes);

  connect(mRxAnalyzerTool, &SAKBaseTool::bytesOutputted, mTcpTransmitterTool,
          &SAKBaseTool::inputBytes);
  connect(mTcpTransmitterTool, &SAKBaseTool::bytesOutputted, mTxAnalyzerTool,
          &SAKBaseTool::inputBytes);
  ...
  connect(mComunicationTool, &SAKCommunicationTool::errorOccured, this,
...
bool SAKToolBox::isWorking() { return mIsWorking; }
...
SAKMaskerTool* SAKToolBox::getRxMaskerTool() { return mRxMaskerTool; }

SAKAnalyzerTool* SAKToolBox::getTxAnalyzerTool() { return mTxAnalyzerTool; }

SAKAnalyzerTool* SAKToolBox::getRxAnalyzerTool() { return mRxAnalyzerTool; }

SAKEmitterTool* SAKToolBox::getEmitterTool() { return mEmitterTool; }

SAKResponserTool* SAKToolBox::getResponserTool() { return mResponserTool; }

SAKStorerTool* SAKToolBox::getStorerTool() { return mStorerTool; }

SAKPrestorerTool* SAKToolBox::getPrestorerTool() { return mPrestorerTool; }

SAKVelometerTool* SAKToolBox::getRxVelometerTool() { return mRxVelometerTool; }

SAKVelometerTool* SAKToolBox::getTxVelometerTool() { return mTxVelometerTool; }
...
QVariant SAKToolBox::txMasker() { return QVariant::fromValue(mTxMaskerTool); }

QVariant SAKToolBox::rxMasker() { return QVariant::fromValue(mRxMaskerTool); }
...
QVariant SAKToolBox::emitter() { return QVariant::fromValue(mEmitterTool); }

QVariant SAKToolBox::responser() { return QVariant::fromValue(mResponserTool); }

QVariant SAKToolBox::storer() { return QVariant::fromValue(mStorerTool); }

QVariant SAKToolBox::prestorer() { return QVariant::fromValue(mPrestorerTool); }
