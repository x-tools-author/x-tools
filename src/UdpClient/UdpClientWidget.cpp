...
UdpClientWidget::UdpClientWidget(QWidget *parent):
    ...
    mpUupClientAssistant(new UupClientAssistant)
{
    mpUupClientAssistant->start();
    /// 初始化UI
    ui->setupUi(this);
...
void UdpClientWidget::Connect()
{
    ...
    connect(this, SIGNAL(Need2Open(QString,QString)), mpUupClientAssistant, SLOT(Open(QString,QString)));
    connect(this, SIGNAL(Need2Close()), mpUupClientAssistant, SLOT(Close()));
    connect(mpUupClientAssistant, SIGNAL(UdpSocketOpenSuccessfully()), this, SLOT(AfterUdpSocketOpen()));
    connect(mpUupClientAssistant, SIGNAL(UdpSocketCloseSuccessfully()), this, SLOT(AfterUdpSocketClose()));
    /// 接收数据
    connect(mpUupClientAssistant, SIGNAL(NewDataHadBeenRead(QByteArray)), mpOutputWindow, SLOT(OutputData(QByteArray)));
    /// 发送数据
    connect(mpSendDataPanel, SIGNAL(Need2WriteData()), this, SLOT(Write()));
    connect(this, SIGNAL(Need2Write(QByteArray, QString, QString)), mpUupClientAssistant, SLOT(Write(QByteArray,QString,QString)));
