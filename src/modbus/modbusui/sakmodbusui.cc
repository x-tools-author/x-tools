...
SAKModbusUi::~SAKModbusUi() { delete ui_; }
...
void SAKModbusUi::InitSignalsDevice() {
  connect(ui_->device_list_,
          ...
          &SAKModbusUi::OnDeviceTypeChanged);
  connect(ui_->open_button_, &QPushButton::clicked, this,
          &SAKModbusUi::OnOpenClicked);
  connect(ui_->cloese_button_, &QPushButton::clicked, this,
          &SAKModbusUi::OnCloseClicked);
}

void SAKModbusUi::InitSignalsNetworking() {
  connect(ui_->address_combo_box, &QComboBox::currentTextChanged, this,
          &SAKModbusUi::OnAddressChanged);
  connect(ui_->port_spin_box, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &SAKModbusUi::OnPortChanged);
}

void SAKModbusUi::InitSignalsSerialPort() {
  connect(ui_->port_name_, &QComboBox::currentTextChanged, this,
          &SAKModbusUi::OnPortNameChanged);
  connect(ui_->parity_, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &SAKModbusUi::OnParityChanged);
  connect(ui_->baud_rate_, &QComboBox::currentTextChanged, this,
          &SAKModbusUi::OnBaudRateChanged);
  connect(ui_->data_bits_, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &SAKModbusUi::OnDataBitsChanged);
  connect(ui_->stop_bits_, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &SAKModbusUi::OnStopBistChanged);
}

void SAKModbusUi::InitSignalsClient() {
  connect(ui_->timeout_, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &SAKModbusUi::OnClientTimeoutChanged);
  connect(ui_->repeat_time_, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &SAKModbusUi::OnClientRepeatTimeChanged);
}

void SAKModbusUi::InitSignalsServer() {
  connect(ui_->device_busy_, &QCheckBox::clicked, this,
          &SAKModbusUi::OnServerIsBusyChanged);
  connect(ui_->listen_only_mode_, &QCheckBox::clicked, this,
          &SAKModbusUi::OnServerJustListenChanged);
  connect(ui_->server_address, QOverload<int>::of(&QSpinBox::valueChanged),
          this, &SAKModbusUi::OnServerAddressChanged);
}

void SAKModbusUi::InitSignalsClientOperations() {
  connect(ui_->function_code_, &QComboBox::currentTextChanged, this,
          &SAKModbusUi::OnFunctionCodeChanged);
  connect(ui_->device_address_, QOverload<int>::of(&QSpinBox::valueChanged),
          this, &SAKModbusUi::OnTargetAddressChanged);
  connect(ui_->start_address_, QOverload<int>::of(&QSpinBox::valueChanged),
          this, &SAKModbusUi::OnStartAddressChanged);
  connect(ui_->quantity_, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &SAKModbusUi::OnAddressNumberChanged);
  connect(ui_->read_, &QPushButton::clicked, this, &SAKModbusUi::OnReadClicked);
  connect(ui_->write_, &QPushButton::clicked, this,
          &SAKModbusUi::OnWriteClicked);
  connect(ui_->send_, &QPushButton::clicked, this, &SAKModbusUi::OnSendClicked);
}
...
void SAKModbusUi::OnOpenClicked() {
  ...
  connect(modbus_device_, &QModbusDevice::errorOccurred, this,
          &SAKModbusUi::OnErrorOccurred);
...
void SAKModbusUi::OnAddressChanged() {
  settings_->setValue(key_ctx_->address, ui_->address_combo_box->currentText());
}

void SAKModbusUi::OnPortChanged() {
  settings_->setValue(key_ctx_->port, ui_->port_spin_box->value());
}

void SAKModbusUi::OnPortNameChanged() {
  settings_->setValue(key_ctx_->port_name, ui_->port_name_->currentText());
}

void SAKModbusUi::OnParityChanged() {
  settings_->setValue(key_ctx_->parity, ui_->parity_->currentIndex());
}

void SAKModbusUi::OnBaudRateChanged() {
  settings_->setValue(key_ctx_->baud_rate, ui_->baud_rate_->currentText());
}

void SAKModbusUi::OnDataBitsChanged() {
  settings_->setValue(key_ctx_->data_bits, ui_->data_bits_->currentIndex());
}

void SAKModbusUi::OnStopBistChanged() {
  settings_->setValue(key_ctx_->stop_bits, ui_->stop_bits_->currentIndex());
}

void SAKModbusUi::OnInvokeRefresh() { InitComponentPortName(); }

void SAKModbusUi::OnClientTimeoutChanged() {
  settings_->setValue(key_ctx_->client_timeout, ui_->timeout_->value());
  UpdateClientParameters();
}

void SAKModbusUi::OnClientRepeatTimeChanged() {
  settings_->setValue(key_ctx_->client_repeat_time, ui_->repeat_time_->value());
  UpdateClientParameters();
}

void SAKModbusUi::OnServerIsBusyChanged() {
  settings_->setValue(key_ctx_->server_is_busy, ui_->device_busy_->isChecked());
  UpdateServerParameters();
}
...
void SAKModbusUi::OnServerAddressChanged() {
  settings_->setValue(key_ctx_->server_address, ui_->server_address->value());
  UpdateServerParameters();
}
...
void SAKModbusUi::OnTargetAddressChanged() {
  settings_->setValue(key_ctx_->target_address, ui_->device_address_->value());
}

void SAKModbusUi::OnStartAddressChanged() {
  settings_->setValue(key_ctx_->start_address, ui_->start_address_->value());
  UpdateClientTableView();
}

void SAKModbusUi::OnAddressNumberChanged() {
  settings_->setValue(key_ctx_->address_number, ui_->quantity_->value());
  UpdateClientTableView();
}
...
void SAKModbusUi::OnWriteClicked() {
  ...
  SAKModbusFactory *factory = SAKModbusFactory::Instance();
  QModbusReply *reply = factory->SendWriteRequest(
...
void SAKModbusUi::OnSendClicked() {
  ...
  SAKModbusFactory *factory = SAKModbusFactory::Instance();
  QModbusReply *reply = factory->SendRawRequest(modbus_device_, server_address,
                                                function_code, data);
  ...
  int index = settings_->value(key_ctx_->send_history_index).toInt();
  bool ret =
      WriteSettingsArray(key_ctx_->send_history, key_ctx_->pdu,
...
void SAKModbusUi::OnDateWritten(QModbusDataUnit::RegisterType table,
  ...
  qCInfo(kLoggingCategory) << "Data written:"
                           << "table:" << table << "start address:" << address
                           << "size:" << size;
  QTableView *tv = GetTableView(table);
  QStandardItemModel *model = qobject_cast<QStandardItemModel *>(tv->model());
  QModbusServer *server = qobject_cast<QModbusServer *>(modbus_device_);
  QList<quint16> data =
      SAKModbusFactory::Instance()->GetServerData(server, table, address, size);
...
void SAKModbusUi::OnItemChanged(QStandardItem *item) {
  ...
  if (SAKModbusFactory::Instance()->IsServerDevice(modbus_device_)) {
    ...
    SAKModbusFactory::Instance()->SetServerData(modbus_device_, table, address,
                                                value);
  }
}
...
QModbusDevice *SAKModbusUi::CreateModbusDevice() {
  ...
  if (SAKModbusFactory::Instance()->IsRtuSerialDeviceType(type)) {
    ...
    SAKModbusFactory *factory = SAKModbusFactory::Instance();
    device = factory->CreateRtuSerialDevice(type, port_name, parity, baud_rate,
                                            data_bits, stop_bits);
...
QTableView *SAKModbusUi::CreateTableView(int row_count,
  ...
  QStringList labels = QStringList()
                       << tr("Address") << tr("Value") << tr("Description");
...
void SAKModbusUi::UpdateServerParameters() {
  ...
  SAKModbusFactory::Instance()->SetServerDeviceParameters(
      modbus_device_, address, device_busy, listen_only_mode);
}
...
void SAKModbusUi::UpdateServerRegistersData() {
  for (int i = 0; i < 4; i++) {
    ...
    for (int row = 0; row < model->rowCount(); row++) {
      ...
      SAKModbusFactory::Instance()->SetServerData(modbus_device_, table, row,
                                                  value, false);
    }
  }
}
...
QList<quint16> SAKModbusUi::GetTableValues(QTableView *table_view, int row,
                                           int count) {
  if (!table_view) {
...
bool SAKModbusUi::IsConnected() {
  ...
  QMessageBox::warning(this, tr("Device is not Ready"),
