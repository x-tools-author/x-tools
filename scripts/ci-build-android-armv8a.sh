...
chmod +x /opt/qt/6.8.3/android_arm64_v8a/bin/qt-cmake
...
#---------------------------------------------------------------------------------------------------
# Build the project
cmake --build build/arm64_v8a --target all --config Release
if [ $? -ne 0 ]; then
...
/opt/qt/6.8.3/gcc_64/bin/androiddeployqt \
    --release \
    --input build/arm64_v8a/android-xTools-deployment-settings.json \
    --output build/arm64_v8a/android-build \
